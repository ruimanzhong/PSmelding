...
# here we show the true sufrace with the sampled data points 
...
```{r warning=FALSE}
res_meld <- fnPredictMelding(depoint = depoint, dearea = a1, dppoint = dppoint, dparea = NULL,
                               boundaryregion = boundaryregion, mesh = mesh, prior.sigma = c(2, 0.1), prior.range = c(0.1, 0.8))


res_ps_meld <- fnMeldingPS(depoint = depoint, dearea = a1, dparea = NULL,prior.sigma = c(2, 0.1), dppoint = dppoint,
                             loc.d = loc.d,
                             prior.range = c(0.1, 0.8),
                             boundaryregion = boundaryregion, mesh = mesh)


res_point <- fnPredictMelding(depoint = depoint, dearea = NULL, dppoint = dppoint, dparea = NULL,boundaryregion = boundaryregion, mesh = mesh, prior.sigma = c(2, 0.01), prior.range = c(0.1, 0.8))
res_ps_point <- fnMeldingPS(depoint = depoint, dearea = NULL, dparea = NULL,prior.sigma = c(2, 0.1), dppoint = dppoint,
                            loc.d = loc.d,
                            prior.range = c(0.1, 0.8),
                            boundaryregion = boundaryregion, mesh = mesh)
```
...
```{r}
res <- makeNamedList(res_meld, res_point,res_ps_meld, res_ps_point)
names <- names(res)
raster_plot(names[1], dppoint)
raster_plot('res_ps_meld', dppoint)
raster_plot(names[2], dppoint)
raster_plot(names[4], dppoint)
```
